...
private $caminho = "sistema/view/";
    ...
    function criaLista() {
        ...
        foreach ($listaEntidades as $entidade) {
            ...
            $dados.="<td><a href='../control/{$entidade}Control.php?{$p3}&{$p2}'>Atualizar</td>\n";
            ...
            foreach ($listaEntidades as $entidade) {
                ...
                foreach ($listaAtributos as $key => $atributo) {
                    if(!$atributo["primary"]) {
                        ...
                        $campos .= "<input value='<?php print (\$alt)?\$obj->get".ucFirst($key)."():\"\";?>' type" . $tipoForm . "' name='" . $key . "' class=\"form-control\">";
                        $campos .= "</div>\n\t";
                    }
                }
                $conteudo = <<<FORM
